...
{
    double sec = (omp_get_wtime() - init);
    if( (sec > timeLimit) || (clq_get_number_of_cliques(clqSet) >= maxCliques) || (it > maxIt))
        return;
...
int BKGraph::execute()
{
   ...
   //cout<<this->maxIt<<" "<<this->maxCliques<<" "<<this->timeLimit<<endl;
...
void BKGraph::setTimelimit(double _timelimit)
{
    timeLimit = _timelimit;
}

void BKGraph::setMaxCliques(int _maxCliques)
{
    maxCliques = _maxCliques;
}
