...
#define CLQ_SEP_DEF_MAX_IT_BK           1000 //INT_MAX/10
#define CLQ_SEP_DEF_CLQE_EXTEND         CLQEM_PRIORITY_GREEDY // CLQEM_EXACT
...
#define CLQ_SEP_DEF_MAX_CLIQUES   INT_MAX
...
struct _CliqueSeparation
{
    ...
    CliqueExtendingMethod extendCliques;
    ...
    /* maximum number of cliques generated by bk */
    int maxCliques;
...
CliqueSeparation *clq_sep_create( const CGraph *origGraph )
{
    ...
    clqSep->maxCliques    = CLQ_SEP_DEF_MAX_CLIQUES;
    ...
    clqSep->extended = xmalloc( sizeof(int)*clqSep->extendedCap );
...
void clq_sep_separate( CliqueSeparation *sep, const double x[] )
{
    ...
    if(cgraph_size(ppcg)>=2)
    {
        ...
        bk_set_max_cliques(sep->bk, sep->maxCliques);
        bk_set_timelimit(sep->bk, sep->maxTimeBK);
        bk_run( sep->bk );
        clock_t endBK = clock();
        if (sep->verbose)
        {
        ...
        if (bkClqSet)
        {
            if (clq_get_number_of_cliques( bkClqSet ))
            {
#ifdef DEBUG
                int nc = clq_get_number_of_cliques( bkClqSet );
                int ic;
                for ( ic = 0 ; (ic<nc) ; ++ic )
                {
                    const ISet *is = clq_set_get_clique( bkClqSet, ic );
                    int n1, n2;
                    if (!clq_validate( ppcg, iset_n_elements(is), iset_elements(is), &n1, &n2 ))
                    {
                        fprintf( stderr, "Nodes %d and %d are not in conflict in ppcg.\n", n1, n2 );
                        exit( EXIT_FAILURE );
                    }
                    int j;
                    for ( j=0 ; (j< iset_n_elements(is)) ; ++j )
                    {
                        const int vidx = iset_element(is,j);
                        assert( vidx >=0 );
    ...
    /* extending cliques */
    vmg_adjust_vector_capacity( (void**)&(sep->extended), &(sep->extendedCap), clq_get_number_of_cliques(clqSetOrig), sizeof(char) );
    int *extended = sep->extended;
    memset( extended, 0, sizeof(int)*clq_get_number_of_cliques( clqSetOrig ) );
    ...
    if (sep->extendCliques)
    {
        ...
        int i;
        for ( i=0 ; (i<clq_get_number_of_cliques( clqSetOrig )) ; ++i )
...
void clq_sep_params_print( const CliqueSeparation *clqsp )
{
    ...
    printf( "\tmaximum time bk   : %f\n",  clqsp->maxTimeBK );
    printf( "\tmaximum cliques bk   : %d\n",  clqsp->maxCliques );
    fflush(stdout);
}
...
void clq_sep_set_maxcliques( CliqueSeparation *sep, int value )
{
    sep->maxCliques = value;
}
