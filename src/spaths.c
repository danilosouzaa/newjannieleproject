...
inline int spf_get_dist( const ShortestPathsFinderPtr spf, const int node )
{
    assert( node < spf->nodes );
    return spf->dist[ node ];
}

inline int spf_get_previous( const ShortestPathsFinderPtr spf, const int node )
{
    assert( node < spf->nodes );
...
inline int spf_fw_get_dist( ShortestPathsFinder* spf, const int i, const int j )
{
#ifdef DEBUG
    assert( spf!=NULL );
    assert( i>=0 );
