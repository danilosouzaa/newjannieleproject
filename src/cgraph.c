...
struct _CGraph
{
    /* per node stored conflicts
       ...
       all node conflicts */
    ISet **nodeConflicts;
    ISet **iNodeCliques;  /* all cliques in which a node appears */
...
CGraph* cgraph_clone(const CGraph *cg)
{
    ...
    clone->iNodeCliques = xmalloc(sizeof(ISet*) * clone->nodeSize);
    for(int i = 0; i < clone->nodeSize; i++) {
        ...
        if(cg->iNodeCliques[i]) {
            clone->iNodeCliques[i] = iset_create(HASH_SIZE);
            iset_cpy(clone->iNodeCliques[i], cg->iNodeCliques[i]);
        } else {
            clone->iNodeCliques[i] = NULL;
        }
    }
...
CGraph *cgraph_preprocess( const CGraph *cgraph, int nindexes[] )
{
    ...
    /* adding new cliques */
    {
        ...
        for ( i=0 ; (i<clq_get_number_of_cliques(cgraph->clqSet)) ; ++i )
        {
            nConflicts = 0;
...
CGraph *cgraph_create( int initialColumns )
{
    ...
    result->iNodeCliques   = xmalloc( sizeof(ISet*)*result->nodeSize );
    {
        int i;
        for ( i=0 ; (i<result->nodeSize) ; ++i ) {
            result->iNodeCliques[i] = NULL;
...
void cgraph_add_node_conflicts( CGraph *cgraph, const int node, const int conflicts[], const int size )
{
    ...
    /* checking node capacity *//* computes the correct degree */
    int lastNode = node;
    for ( i=0 ; (i<size) ; ++i )
...
void cgraph_add_clique( CGraph *cgraph, int clique[], const int size )
{
    ...
    {
        int i;
        const int idxClique = clq_get_number_of_cliques(cgraph->clqSet) - 1;
        for ( i=0 ; (i<size) ; ++i )
        {
            cgraph_check_node_size( cgraph, clique[i]+1 );

            //check if iset was not created
            if(!cgraph->iNodeCliques[clique[i]])
                cgraph->iNodeCliques[clique[i]] = iset_create(HASH_SIZE);

            iset_add(cgraph->iNodeCliques[clique[i]], idxClique);
        }
    }
}
...
void cgraph_check_node_size( CGraph *cgraph, int requiredNodeSize )
{
    ...
    cgraph->iNodeCliques = xrealloc( cgraph->iNodeCliques, sizeof(ISet*)*newNodeSize );
...
int cgraph_get_all_conflicting( const CGraph *cgraph, int node, int neighs[], int maxSize )
{
    ...
    const int nCliques = (cgraph->iNodeCliques[node]) ? iset_n_elements(cgraph->iNodeCliques[node]) : 0;
    for(currClique = 0; currClique < nCliques; currClique++)
    {
        const int idxClique = iset_element(cgraph->iNodeCliques[node], currClique);
...
void cgraph_save( CGraph *cgraph, const char *fileName )
{
...

    {
        ...
        for ( i=0 ; (i<cgraph_size(cgraph)) ; ++i )
        {
            ...
            for ( j=0 ; (j<nsize) ; ++j ) {
                const int neigh = iset_element(is, j);
                if (neigh > i)
                    fprintf(f, "e %d %d\n", i + 1, neigh + 1);
            }
        }
...
CGraph *cgraph_load( const char *fileName )
{
...

    {
        ...
        int i;
        for ( i=0 ; (i<clq_get_number_of_cliques(clqSet)) ; ++i )
        {
...
void cgraph_free( CGraph **cgraph )
{
    ...
    for ( i=0 ; (i<(*cgraph)->nodeSize) ; ++i ) {
        ...
        if((*cgraph)->iNodeCliques[i])
            iset_free(&(*cgraph)->iNodeCliques[i]);
    }

    free( (*cgraph)->nodeConflicts );
    free( (*cgraph)->iNodeCliques );
...
CGraph *cgraph_create_induced_subgraph( const CGraph *cgraph, const int nindexes[] )
{
    ...
    /* filling new cliques */
    const int nCliques = clq_get_number_of_cliques( cgraph->clqSet );
...
void cgraph_check_node_cliques( const CGraph *cgraph )
{
    ...
    for ( i=0 ; (i<cgraph_size(cgraph)) ; ++i )
    {
        const int nCliques = iset_n_elements(cgraph->iNodeCliques[i]);
        const int *el = iset_elements( cgraph->iNodeCliques[i] );
    ...
    const int nc = clq_get_number_of_cliques( cgraph->clqSet );
    for ( i=0 ; (i<nc) ; ++i )
    {
        ...
        for ( j=0 ; (j<n) ; ++j )
        {
            ...
            int l;
            int nn = iset_n_elements( cgraph->iNodeCliques[currNode] );
            const int *eel = iset_elements( cgraph->iNodeCliques[currNode] );
...
void nit_fill_bhv( NeighIterator *nit, const CGraph *cgraph, int node, const int costs[] )
{
...

    {
        ...
        for ( i=0 ; (i<nCliques) ; ++i )
        {
            const int cliqueIdx = iset_element(cgraph->iNodeCliques[node], i);
...
void nit_start( NeighIterator *nit, const CGraph *cgraph, int node, const int costs[] )
{
    ...
    int nCliques       = (cgraph->iNodeCliques[node]) ? iset_n_elements(cgraph->iNodeCliques[node]) : 0;
    const CliqueSet *clqSet = cgraph->clqSet;
    {
        int i;
        for ( i=0 ; (i<nCliques) ; ++i ) {
            const int clique = iset_element(cgraph->iNodeCliques[node], i);
...
void cgraph_set_weight( CGraph *cgraph, int* weight )
{
    if (!cgraph->w)
        cgraph->w = xmalloc( sizeof(int)*cgraph_size(cgraph));
    int node ;
    for(node= 0 ; node  < cgraph_size(cgraph) ; node++ )
        cgraph->w[node] = weight[node];
}
