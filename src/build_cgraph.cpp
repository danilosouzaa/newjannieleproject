...
void fetchConflicts( const bool lastTime, CGraph *cgraph, vector< pair< int, int > > &cvec)
{
    ...
    vector<pair<int, int> >::const_iterator vIt;
    for(vIt = cvec.begin(); vIt != cvec.end(); ++vIt)
        cgraph_add_node_conflict(cgraph, vIt->first, vIt->second);
...
{
    CGraph *cgraph = cgraph_create(nVertices);
    ...
    {
        for(j = 0; j < nconf[i]; j++)
            cvec.push_back(pair<int,int>(i, conf[i][j]));
        fetchConflicts(false, cgraph, cvec);
        _time = ( (double) runtime - (omp_get_wtime()-startT) );
        if(_time < 3){
           // printf("time %f runtime %f currtime %f\n", _time, runtime,(omp_get_wtime()-startT) );
            break;
        }
    }

    fetchConflicts(true, cgraph,cvec); //&cvec *cvec
    cgraph_update_min_max_degree( cgraph );
