...
VStr *vstr_create()
{
    ...
    ALLOCATE( vstr, VStr );
    vstr->capacity = 64;
    vstr->charCap = vstr->capacity*64;
    ALLOCATE_VECTOR( vstr->sv, char *, vstr->capacity );
    ALLOCATE_VECTOR( vstr->s, char, vstr->charCap );
    vstr->sv[0] = vstr->s;
...
void vstr_push_back( VStr *vstr, const char str[] )
{
    int len = strlen(str);
 
    // need resize s
    if ( vstr->charSize+len+1 > vstr->charCap )
    {
        vstr->charCap = vstr->charCap*2;
        char *s = (char*) xrealloc( vstr->s, vstr->charCap*sizeof(char) );
        char *cs = s;
        for ( int i=1 ; (i<vstr->size) ; ++i )
        {
            char *thisCS = cs;
            cs += vstr->sv[i]-vstr->sv[i-1];
            vstr->sv[i-1] = thisCS;
        }
        vstr->sv[vstr->size-1] = cs;
        vstr->sv[vstr->size] = vstr->sv[vstr->size-1] + strlen(vstr->sv[vstr->size-1]) + 1;
        vstr->s = s;
    }

    // need resize sv
    if (vstr->size+2 > vstr->capacity)
    {
        vstr->capacity = vstr->capacity*2+1;
        char **sv = (char**)xrealloc( vstr->sv, vstr->capacity*(sizeof(char*)) );
        vstr->sv = sv;
    }

    strcpy(vstr->sv[vstr->size], str);

    vstr->charSize += len+1;
    vstr->size++;

    vstr->sv[vstr->size] = vstr->sv[vstr->size-1] + len + 1;
}

const char *vstr_get( const VStr *vstr, int i )
{
#ifdef DEBUG
    assert( i>=0 && i<vstr->size );
#endif
    return vstr->sv[i];
}

char **vstr_p( const VStr *vstr )
{
    return vstr->sv;
}

void vstr_free( VStr **_vstr )
{
    VStr *vstr = *_vstr;
    free( vstr->s );
    free( vstr->sv );
...
void iset_resize_hash( ISet *iset, int newHashSize )
{
    //printf("\nRESIZE HASH.\n");
    assert( newHashSize > iset->hashSize );
    ISet *tmpSet = iset_create( newHashSize );

    /* add to temporary hash */
    for ( int i=0 ; (i<iset_n_elements(iset)) ; ++i )
        iset_add( tmpSet, iset_element(iset,i) );

    /* clearing and populating again this hash */
    iset->hashSize = newHashSize;
    
    iset_clear_mem( iset );

    iset->elements = vec_int_create_cap( iset_n_elements(tmpSet)*3 );
    vec_int_cpy( iset->elements, tmpSet->elements );
    iset->rows = vec_Vec_IntPairp_create_fill( newHashSize, NULL );

    for ( int i=0 ; (i<newHashSize) ; ++i )
    {
        Vec_IntPair *tmpVip = vec_Vec_IntPairp_get( tmpSet->rows, i );
        if ( tmpVip && (vec_IntPair_size(tmpVip)>0) )
        {
            Vec_IntPair *vip = vec_IntPair_create_cap( vec_IntPair_size(tmpVip)*2 );
            vec_IntPair_cpy( vip, tmpVip );
            vec_Vec_IntPairp_set( iset->rows, i, vip );
        }
    }
...
{
    if (iset_n_elements(set)>=set->hashSize*8)
    {
        if (iset_n_elements(set)<64)
        {
            iset_resize_hash( set, set->hashSize*8 );
        }
        else
        {
            if (iset_n_elements(set)<4096)
            {
                iset_resize_hash( set, set->hashSize*2 );
            }
            else
            {
                iset_resize_hash( set, (int)ceil(((double)set->hashSize)*1.5) );
            }
        }
    }
...
void iset_add_using_original_indexes( ISet *iset, const int elements[], int size, const int orig[] )
{
    const int newSize = iset_n_elements(iset) + size;

    for(int i = 0; i < size; i++) {
        iset_add(iset, orig[elements[i]]);
    }
}
...
const int *iset_elements( const ISet *set )
{
    return vec_int_ptr( set->elements );
}

int iset_cmp_int( const void *e1, const void *e2 )
{
    int i1 = (*((const int*) e1));
    int i2 = (*((const int*) e2));
