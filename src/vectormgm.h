...
#define ADJUST_CHAR_VECTOR_CAPACITY( vector, capacity, required ) \
{\
   if ( (capacity) < (required) ) \
   { \
      (capacity) *= 2; \
      if ( (capacity) < (required) ) \
         (capacity) = (required); \
      char *temp = realloc( vector, (capacity)*sizeof(char) ); \
      if (!temp) \
      { \
         fprintf( stderr, "Error trying to allocate a vector with %d char. No more memory available. Error at %s:%d.\n", (required), __FILE__, __LINE__ ); \
         exit( EXIT_FAILURE ); \
      } \
      vector = temp; \
   } \
