...
int clqe_get_best_candidates_clique_insertion( CliqueExtender *clqe, const ISet *clique,
        const CliqueExtendingMethod clqem )
{
    /* node with the smallest degree */
    int nodeSD = -1, degree = INT_MAX, i;

    const int cliqueSize = iset_n_elements( clique ), *cliqueEl = iset_elements( clique );
...
int exact_clique_extension( CliqueExtender *clqe, const ISet *clique, const int weight )
{
    ...
    const int clqSize = iset_n_elements(clique);
    const int *clqEl = iset_elements(clique);
...
//    assert( (cgSize % 2) == 0 );
    ...
    const int numClqs = clq_get_number_of_cliques(clqSet);
    if(clqSet && numClqs)
    {
...
int clqe_extend( CliqueExtender *clqe, const CGraph *cgraph, const ISet *clique,
                 const int weight, const CliqueExtendingMethod clqem )
{
    int result = 0;

#ifdef DEBUG
    assert( (clqe) && (cgraph) && (clique) && ((iset_n_elements(clique))) );
    int en1, en2;
    if (!clq_validate( cgraph, iset_n_elements(clique), iset_elements(clique), &en1, &en2 ))
    {
    ...
    memcpy( clqe->newClique, iset_elements( clique ), sizeof(int)*iset_n_elements(clique) );
    clqe->newCliqueSize = iset_n_elements( clique );
