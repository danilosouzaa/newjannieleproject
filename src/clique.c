...
int clq_get_number_of_cliques(const CliqueSet *clqSet) {
    if (!clqSet)
        return 0;
...
int clq_set_clique_already_inserted(const CliqueSet *clqSet, const int n, const int idx[]) {
    ...
    const int hashEntrySize = iset_n_elements(hashEntry);
    ...
    for (i = 0; (i < hashEntrySize); ++i) {
        ...
        if (int_vector_equals(n, idx, iset_n_elements(otherClique), iset_elements(otherClique)))
            return 1;
    }
...
int clq_set_add_cliques(CliqueSet *clqs_target, const CliqueSet *clqs_source) {
    ...
    {
        int i;
        for (i = 0; (i < clq_get_number_of_cliques(clqs_source)); ++i) {
...
void clq_set_add_using_original_indexes(CliqueSet *target, const CliqueSet *source, const int orig[]) {
    int i;
    for (i = 0; (i < clq_get_number_of_cliques(source)); ++i) {
