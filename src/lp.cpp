...
/*1: callback with just precedence cuts
  2: callback with just clique cuts
  3: callback with precedence cuts and clique cuts
*/
#define NCALLBACK 1
...
#ifdef CBC
void getcallback(LinearProgramPtr lp, double time){
    lp_set_callback( lp, lp_cbc_callback, (void*) &time);
}

int lp_cbc_callback( LinearProgram* lp, int whereFrom, const int *origCols, LinearProgram *origLP, void *data)
{
  //  double time;
    double runTime;

    OsiSolverInterface *linearProgram = linearProgram = lp->osiLP;
   //time = *((double*)usrdata);
    int nodelimitlocal =0;
   // double timeLeft = *(double*)usrdata;
    CbcModel modelenv(*linearProgram);
   // printf("wherefrom %d ", whereFrom); fflush(stdout); getchar();
    if (whereFrom == LP_OPTIMAL || whereFrom == LP_FEASIBLE) {
    /* MIP solution callback */
        double nodecnt, obj, runTime;
        int    solcnt;
        nodecnt = modelenv.getNodeCount();
        obj = modelenv.getObjValue();
        solcnt = modelenv.getSolutionCount();

        if(solcnt>=1){
            nodelimitlocal = nodecnt+25;
            modelenv.setMaximumNodes(nodelimitlocal);
            //        printf("**** New solution at node %.0f, obj %g, sol %d, runTime %f nodelimitlocal %d  ****\n",// x[0] = %.2f
             //  nodecnt, obj, solcnt, runTime,nodelimitlocal);//, mydata->solution[0]);
        }
    }
    return 0;
}
...
void lp_read(LinearProgram *lp, const char *fileName)
{
    ...
    grbError = GRBreadmodel(LPgrbDefaultEnv, fileName, &(lp->lp) );
    lp_check_for_grb_error(LPgrbDefaultEnv, grbError, __FILE__, __LINE__);
    return;
#endif
...
#ifdef GRB
void lp_conflicts_model(LinearProgram *lp){
    int conf = GRBcomputeIIS(lp->lp);
    GRBwrite(lp->lp,"mipCut.ilp");
  //  printf("%s\n", GRBgeterrormsg());
}

void lp_set_numeric_focus(LinearProgram *lp, int value){
    int error = GRBsetintparam(GRBgetenv(lp->lp), GRB_INT_PAR_NUMERICFOCUS, value);
    lp_check_for_grb_error( LPgrbDefaultEnv, error, __FILE__, __LINE__ );
}

void lp_set_method(LinearProgram *lp, int value){
    ...
    int error = GRBsetintparam(GRBgetenv(lp->lp), GRB_INT_PAR_METHOD, value);
    lp_check_for_grb_error( LPgrbDefaultEnv, error, __FILE__, __LINE__ );
}

void lp_set_seed(LinearProgram *lp, int value){
    int error = GRBsetintparam(GRBgetenv(lp->lp), GRB_INT_PAR_SEED, value);
    lp_check_for_grb_error( LPgrbDefaultEnv, error, __FILE__, __LINE__ );
}

void lp_set_concurrentMIP(LinearProgram *lp, int value){
    int error = GRBsetintparam(GRBgetenv(lp->lp), GRB_INT_PAR_CONCURRENTMIP, value);
    lp_check_for_grb_error( LPgrbDefaultEnv, error, __FILE__, __LINE__ );
}
...
static void lp_validate_row_data(LinearProgram *lp, const int nz,  int *indexes, double *coefs, const char *name, char sense, const double rhs)
{
    ...
    for (int i = 0 ; (i < nz) ; ++i) {
        ...
        for ( int j=i+1 ; j<nz ; ++j )
        {
            ...
            {
                    //   printf("\nindexes[%d] %d,indexes[%d] %d", i,  indexes[i],j, indexes[j]);
#ifdef GRB
                flush_model_updates( lp );
#endif
                char varName[256];
...
void lp_add_cut_grb( void *cbdata, int nz, int *cutIdx, double *cutCoef, char sense, double rhs )
{
#ifdef GRB
    int grbError = GRBcbcut(cbdata, nz, cutIdx, cutCoef, sense, rhs);
    lp_check_for_grb_error( LPgrbDefaultEnv, grbError, __FILE__, __LINE__ );
#endif
}
...
double lp_get_gap(LinearProgram *lp){
#ifdef GRB
    double gap = DBL_MAX;
    int grbError = GRBgetdblattr(lp->lp, GRB_DBL_ATTR_MIPGAP, &gap);
    lp_check_for_grb_error( LPgrbDefaultEnv, grbError, __FILE__, __LINE__ );
    return gap;
#endif // GRB
}

double lp_get_number_nodes_explored(LinearProgram *lp){
#ifdef GRB
    double nnode = DBL_MAX;
    int grbError = GRBgetdblattr(lp->lp, GRB_DBL_ATTR_NODECOUNT, &nnode);
    lp_check_for_grb_error( LPgrbDefaultEnv, grbError, __FILE__, __LINE__ );
    return nnode;
#endif // GRB
}
...
/*int lp_get_number_number_cut(LinearProgram *lp){
#ifdef GRB
    int ncuts = DBL_MAX;
    int grbError = GRBcbget(lp->lp, GRB_INT_ATTR_CUTCNT, &ncuts);
    lp_check_for_grb_error( LPgrbDefaultEnv, grbError, __FILE__, __LINE__ );
    return nnode;
#endif // GRB
}*/
...
//int lp_strengthen_with_cuts( LinearProgram *lp, const int maxRoundsCuts[], Results *res );
int lp_strengthen_with_cuts( LinearProgram *lp, const int maxRoundsCuts[])//  int nCut[] )
{
    OsiClpSolverInterface *osiLP = lp->_lp;
    int round = 1;
    int totalCuts = 0;
...

    {
        ...
        if (round<=maxRoundsCuts[LPC_FLOW])
        {
            ...
            if (cuts.sizeCuts()>oc)
            {
                ...
                strcat( message, str );
                //nCut[LPC_FLOW] = cuts.sizeCuts()-oc;
//                if(VERBOSE) Res_setTCutsTotal(res, round_res,LPC_FLOW,cuts.sizeCuts()-oc);
            }
        }
        ...
        if (round<=maxRoundsCuts[LPC_ZERO_HALF])
        {
            ...
            if (cuts.sizeCuts()>oc)
            {
                ...
                strcat( message, str );
                //nCut[LPC_ZERO_HALF] += cuts.sizeCuts()-oc;
//                if(VERBOSE) Res_setTCutsTotal(res,round_res,LPC_ZERO_HALF,cuts.sizeCuts()-oc);
            }
        }
        ...
        if (round<=maxRoundsCuts[LPC_MIR])
        {
            ...
            if (cuts.sizeCuts()>oc)
            {
                ...
                strcat( message, str );
//                if(VERBOSE) Res_setTCutsTotal(res,round_res,LPC_MIR,cuts.sizeCuts()-oc);
                //nCutsTotal[LPC_MIR] += cuts.sizeCuts()-oc;
            }
        }
        ...
        if (round<=maxRoundsCuts[LPC_GOMORY])
        {
            ...
            if (cuts.sizeCuts()>oc)
            {
                ...
                strcat( message, str );
//                if(VERBOSE) Res_setTCutsTotal(res,round_res,LPC_GOMORY,cuts.sizeCuts()-oc);
                //nCutsTotal[LPC_GOMORY] += cuts.sizeCuts()-oc;
            }
        }
        ...
        if (round<=maxRoundsCuts[LPC_KNAPSACK])
        {
            ...
            if (cuts.sizeCuts()>oc)
            {
                ...
                strcat( message, str );
                //nCutsTotal[LPC_KNAPSACK] += cuts.sizeCuts()-oc;
//                if(VERBOSE) Res_setTCutsTotal(res,round_res,LPC_KNAPSACK,cuts.sizeCuts()-oc);
            }
        }
        ...
        if (round<=maxRoundsCuts[LPC_TWO_MIR])
        {
            ...
            if (cuts.sizeCuts()>oc)
            {
                ...
                strcat( message, str );
//                if(VERBOSE) Res_setTCutsTotal(res,round_res,LPC_TWO_MIR,cuts.sizeCuts()-oc);
                //nCutsTotal[LPC_TWO_MIR] += cuts.sizeCuts()-oc;
            }
        }
        ...
        if (round<=maxRoundsCuts[LPC_L_AND_P])
        {
            ...
            if (cuts.sizeCuts()>oc)
            {
                ...
                strcat( message, str );
                //nCutsTotal[LPC_L_AND_P] += cuts.sizeCuts()-oc;
//                if(VERBOSE) Res_setTCutsTotal(res,round_res,LPC_L_AND_P,cuts.sizeCuts()-oc);
            }
        }
        ...
        if (round<=maxRoundsCuts[LPC_REDUCE])
        {
            ...
            if (cuts.sizeCuts()>oc)
            {
                ...
                strcat( message, str );
                //nCutsTotal[LPC_REDUCE] += cuts.sizeCuts()-oc;
//                if(VERBOSE) Res_setTCutsTotal(res,round_res,LPC_REDUCE,cuts.sizeCuts()-oc);
            }
        }
...
//janniele
void lp_set_obj_elem(LinearProgram *lp, int elem, double value)
{
    assert(lp != NULL);

#ifdef GRB
    int grbError = GRBsetdblattrelement(lp->lp, "Obj", elem, value);
    lp_check_for_grb_error( LPgrbDefaultEnv, grbError, __FILE__, __LINE__ );
    grbError = GRBupdatemodel( lp->lp );
    lp_check_for_grb_error( LPgrbDefaultEnv, grbError, __FILE__, __LINE__ );
#endif
...
#ifdef  CBC
void lp_set_ownCutPool(LinearProgram *lp, char value)
{
    assert(lp != NULL);
    lp->ownCutPool = value;
    if(lp->ownCutPool)
        lp->cutPool = new OsiCuts();
...
void lp_set_nThreads(LinearProgram *lp, int value)
{
    assert(lp != NULL);
    lp->parallel = value;
}
...
void lp_config_grb_params(LinearProgram *lp)
{
    ...
    if ( lp->parallel != INT_NOT_SET){
        if(lp->parallel)
            GRBsetintparam( env, GRB_INT_PAR_THREADS, 4 );
        else
...
void lp_fix_mipstart( LinearProgram *lp )
{
    ...
    for ( int i=0 ; (i<lp->msVars) ; ++i )
    {
        ...
        //lp_set_concurrentMIP(lp,1);
        //lp_set_method(lp,4);
        //lp_set_seed(lp,100000);
...
//janniele
/*double lp_current_obj( LinearProgram *lp )
{
    return lp->currentObj;
}*/

void lp_set_max_reduced_cost(LinearProgram *lp, double _max)//janniele
{
    assert(lp != NULL);
    lp->maxReducedCost = _max;
}

double lp_get_max_reduced_cost(LinearProgram *lp)//janniele
{
    assert(lp != NULL);
    return lp->maxReducedCost;
}


#ifdef GRB

/*structure to pass and save informations inside callback procedure*/
DataGRBCallback *DataGRB_create( const Instance *inst,  double timeLeft, int lifting, double maxcuts){
    assert(inst);

    DataGRBCallback *dt = (DataGRBCallback*)malloc(1);


    dt->timeLeft=timeLeft;
    dt->lifting=lifting;
    dt->maxcuts=maxcuts;
    dt->inst=inst;
    dt->nCuts = 0;
    dt->nNoImp = 0;
    dt->oldbestlb =0.0;
   // printf("data nJobs %d", Inst_nJobs(dt->inst)); getchar();
    return dt;
}

int DataGRB_getNCuts( DataGRBCallback *usrdata ){
    return usrdata->nCuts;
}

/*set callback function in Gurobi
...
*/
int lp_grb_cut_getcallback(LinearProgramPtr lp, DataGRBCallback *usrdata){

    GRBmodel *grbmmodel = lp_get_grbmodel(lp);
    GRBenv *modelenv = GRBgetenv(grbmmodel);
    ...
    GRBsetintparam(modelenv, GRB_INT_PAR_PRECRUSH, 1);
    //int erro = GRBsetcallbackfunc(grbmmodel, lp_grb_callback_prec, usrdata);
 //   int erro = GRBsetcallbackfunc(grbmmodel, lp_grb_callback_improvelb, usrdata);
int erro =0;
    return erro;
}


/*search for new cuts when lower bound improve: Callback to Precedence and Clique cut*/
int lp_grb_callback_improvelb(GRBmodel *model,  void *cbdata, int where, void *usrdata)
{

    DataGRBCallback *dt = ( DataGRBCallback *) usrdata;
    int status = -1;
    GRBcbget(cbdata, where, GRB_CB_MIPNODE_STATUS, &status);

    if ((where == GRB_CB_MIPNODE) && (status == GRB_OPTIMAL)){ // if node with optimal fractional solution
      //  printf("status %d, GRB_OPT %d\n", status, GRB_OPTIMAL);
        double improve=0.0;
        GRBcbget(cbdata, where, GRB_CB_MIPNODE_OBJBND, &improve);
        if(improve > dt->oldbestlb+EPS) {
            dt->oldbestlb = improve;
            switch(NCALLBACK){
                case 1:{
                    CutPR_add_cuts_precedence_callback( model, cbdata,  dt->inst, &dt->timeLeft,dt->lifting,dt->maxcuts, &dt->nCuts);
                    break;
                }
                case 2:{
                    CutCL_add_cuts_clique_callback( model, cbdata,  dt->inst, &dt->timeLeft,dt->lifting,dt->maxcuts, &dt->nCuts);
                    break;
                }
                case 3:{
                    CutPR_add_cuts_precedence_callback( model, cbdata,  dt->inst, &dt->timeLeft,dt->lifting,dt->maxcuts, &dt->nCuts);
    ...
    //printf("Printing lp in callback...");
    //GRBwrite(model,"A-4_callback.lp"); getchar();
    return 0;
}


GRBmodel * lp_get_grbmodel(LinearProgramPtr lp ){
    return lp->lp;
}


int getcallback(LinearProgramPtr lp, double time){
    int erro = GRBsetcallbackfunc(lp->lp, lp_grb_callback, (void*) &time);
    return erro;
}

int lp_grb_callback(GRBmodel *model, void *cbdata, int where, void *usrdata)
{
  //  double time;
//    double runTime;
     ...
     //  GRBcbget(cbdata, where, GRB_CB_RUNTIME, &runTime);
      // time = time - runTime;
       // printf("%f\n", time);
        ...
        if(solcnt>=1){
            nodelimitlocal = nodecnt+25;
            GRBsetdblparam(modelenv, GRB_DBL_PAR_NODELIMIT, nodelimitlocal );
                //    printf("**** New solution at node %.0f, obj %g, sol %d, time %f nodelimitlocal %d  ****\n", nodecnt, obj, solcnt, time,nodelimitlocal);//, mydata->solution[0]);
        }
...
/*double lp_best_possible_obj( LinearProgram *lp )
{
    return lp->bestPossibleObj;
}*/
